...
void ParticlePlaneCollision::do_solveCollision()
{
    ...
    //float d2Plane = std::min( abs(glm::dot(p,n) - d) - r, 0.0f);
    float d2Plane = abs(glm::dot(p,n) - d );

    //Project the particle on the plane
    glm::vec3 correctionPos = p - (d2Plane - r) * n;
    m_particle->setPosition(correctionPos);

    //Compute post-collision velocity
    glm::vec3 correctionVel = v - (1.0f + m_restitution) * (glm::dot(v,n)) * n;
    m_particle->setVelocity(correctionVel);
}
